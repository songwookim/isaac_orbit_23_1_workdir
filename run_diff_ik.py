...
class TableTopSceneCfg(InteractiveSceneCfg):
    ...
    # articulation
    if args_cli.robot == "kinova":
        robot = KINOVA_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    ee_goals = [
        [0.5, 0, 0.055, 1, 0, 0., 0],
        # [0.5, -0.4, 0.6, 0.707, 0.707, 0.0, 0.0],
        # [0.5, 0, 0.5, 0.0, 11 - torch.tanh(object_ee_distance / std).0, 0.0, 0.0],
    ]
    ...
    if args_cli.robot == "kinova":
        robot_entity_cfg = SceneEntityCfg("robot", joint_names=["j2n6s300_joint.*"], body_names=["j2n6s300_link_finger_.*"])
    elif args_cli.robot == "ur10":
